...
const FullPhotoEditorPage: React.FC<FullPhotoEditorPageProps> = ({ onNavigate, currentPage, allTools }) => {
  ...
  const renderPanel = () => (
    <div className="w-full bg-gray-800/50 border border-gray-700 rounded-lg p-4 flex flex-col gap-4 animate-fade-in backdrop-blur-sm max-w-4xl">
        <h3 className="text-xl font-bold text-center">Full Photo Editor</h3>
        <p className="text-sm text-center text-gray-400">This all-in-one editor provides powerful, selection-based generative AI tools. Use it to add, remove, or change parts of your image with precision. For one-click filters and other specialized tools, please visit their dedicated pages from the homepage.</p>
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleGenerativeEdit(); }} className="w-full flex items-center gap-2">
            <input
              ...
              placeholder={selection ? "e.g., 'remove this object' or 'change shirt to blue'" : "First make a selection on the image"}
