                        ...
                        <div className="flex flex-col lg:flex-row gap-8">
                            ...
                            <div className="w-full lg:w-1/3 flex flex-col gap-6">
                                ...
                                <div className="grid grid-cols-4 items-center gap-2 bg-black/20 p-1 rounded-md">
                                    {[{id: 'rectangle', label: 'Rectangle', Icon: RectangleIcon},{id: 'lasso', label: 'Lasso', Icon: LassoIcon},{id: 'magicWand', label: 'Magic Wand', Icon: WandIcon}].map(t => (
                                        <button key={t.id} onClick={() => setActiveTool(t.id as SelectionTool)} className={`flex flex-col items-center justify-center gap-1 w-full font-semibold py-2 px-3 rounded-md transition-all duration-200 text-xs ${activeTool === t.id ? 'bg-gray-600 text-white' : 'text-gray-400 hover:bg-white/10'}`} title={t.label}><t.Icon className="w-6 h-6"/></button>
                                    ))}
                                    <button onClick={() => setSelection(null)} disabled={!selection || isLoading} className="flex flex-col items-center justify-center gap-1 w-full font-semibold py-2 px-3 rounded-md transition-all text-xs text-gray-400 hover:bg-white/10 disabled:opacity-50" title="Clear Selection"><ClearSelectionIcon className="w-6 h-6"/></button>
                                </div>
                                
                                {activeTool === 'magicWand' && (
                                    <div className="w-full bg-black/20 p-3 rounded-lg flex flex-col gap-2 animate-fade-in">
                                    <div className="flex items-center justify-between">
                                        <label htmlFor="tolerance-slider" className="font-semibold text-gray-200">Tolerance</label>
                                        <span className="text-sm font-mono bg-gray-700/50 px-2 py-1 rounded">{tolerance}</span>
                                    </div>
                                    <input id="tolerance-slider" type="range" min="1" max="200" step="1" value={tolerance} onChange={(e) => setTolerance(Number(e.target.value))} disabled={isLoading} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer" title="Adjust color sensitivity"/>
                                    </div>
                                )}
